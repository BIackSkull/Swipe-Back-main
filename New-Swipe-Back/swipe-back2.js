...
if (!document.querySelector(`.${CONTAINER_CLASS}`)) {
  ...
  // MODIFIED: createArrow now accepts an image source
  function createArrow(classNames, imgSrc) {
    const img = document.createElement("img");
    img.src = imgSrc; // Use the provided image source
    img.className = classNames;
    return img;
  }

  // MODIFIED: Use IMAGE_LEFT_ARROW for left and IMAGE_RIGHT_ARROW for right
  const leftArrow = createArrow("browser-extension-swipe-back-arrow browser-extension-swipe-back-arrow-left", IMAGE_LEFT_ARROW);
  const rightArrow = createArrow("browser-extension-swipe-back-arrow browser-extension-swipe-back-arrow-right", IMAGE_RIGHT_ARROW);
  ...
  function handleWheel(event) {
    ...
    if (position < 0) {
      showArrow(rightArrow); // Show right arrow for forward swipe
    } else if (position > 0) {
      showArrow(leftArrow); // Show left arrow for back swipe
    }
